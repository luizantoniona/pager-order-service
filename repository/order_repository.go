...
func InsertOrder(order model.Order) error {
	...
	query, err := database.ReadSQLFile("./repository/sql/order/insert_order.sql")
	if err != nil {
		tx.Rollback()
		return err
	}
	...
	query, err = database.ReadSQLFile("./repository/sql/order_customer/insert_order_customer.sql")
	if err != nil {
		tx.Rollback()
		return err
	}
	...
	query, err = database.ReadSQLFile("./repository/sql/order_address/insert_order_address.sql")
	if err != nil {
		tx.Rollback()
		return err
	}
	...
	for _, item := range order.Items {
		query, err = database.ReadSQLFile("./repository/sql/order_item/insert_order_item.sql")
		if err != nil {
			tx.Rollback()
			return err
		}
